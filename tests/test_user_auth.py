...
class TestUserAuth():
    ...
    def setup(self):
        ...
        assert "auth_sid" in response1.cookies, "There is no auth_sid in the response"
    ...
    def test_auth_user(self):
        ...
        assert "user_id" in response2.json(), "There is no user id in response"

        user_id_from_check_method = response2.json()["user_id"]
        print(user_id_from_check_method)
        assert self.user_id_from_auth_method == user_id_from_check_method, "User id's not equal"
    ...
    def test_negative_auth_check(self, condition):
        ...
        assert "user_id" in response2.json(), "There is no user_id"
